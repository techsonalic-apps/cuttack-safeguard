...
const App: React.FC = () => {
  ...
  const SOSView = () => (
    <div className={`flex flex-col h-full transition-colors duration-500 overflow-y-auto ${isStrobeActive ? 'animate-strobe' : settings.silentMode ? 'bg-slate-900' : 'bg-slate-50'}`}>
        ...
        >
          <div className="flex items-center gap-3">
            <div className="p-2 bg-blue-600 rounded-xl">
              <InstallIcon className="w-5 h-5 text-white" />
            </div>
            <div className="text-left">
              <span className="font-black text-xs block uppercase tracking-tight">Direct Install App</span>
              <span className="text-[9px] font-bold opacity-60 uppercase tracking-widest">Pin to home screen</span>
            </div>
          </div>
      ...
      {/* MAP BUTTONS - ENSURED VISIBILITY */}
      <div className="px-4 mb-4">
        <div className="flex items-center gap-2 mb-3 px-1">
          <MapPin className="w-4 h-4 text-slate-400" />
          <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Nearby Safe Places</span>
        </div>
        <div className="grid grid-cols-2 gap-3">
          <button 
            onClick={() => handleNearbySearch('Police')} 
            className={`p-5 rounded-3xl border flex flex-col items-center justify-center gap-2 transition-all active:scale-95 shadow-sm ${settings.silentMode ? 'bg-slate-800 border-slate-700 text-slate-400' : 'bg-white border-blue-100 text-blue-600 hover:bg-blue-50'}`}
          >
            <Building2 className="w-8 h-8" />
            <span className="text-[11px] font-black uppercase tracking-wider">Search Police</span>
          </button>
          <button 
            onClick={() => handleNearbySearch('Hospital')} 
            className={`p-5 rounded-3xl border flex flex-col items-center justify-center gap-2 transition-all active:scale-95 shadow-sm ${settings.silentMode ? 'bg-slate-800 border-slate-700 text-slate-400' : 'bg-white border-red-100 text-red-600 hover:bg-red-50'}`}
          >
            <Stethoscope className="w-8 h-8" />
            <span className="text-[11px] font-black uppercase tracking-wider">Search Hospital</span>
          </button>
        </div>
      </div>
      ...
      <div className="px-4 pb-12 space-y-3">
        <button onClick={() => window.location.href='tel:112'} className={`w-full p-5 rounded-3xl flex items-center justify-center gap-3 font-black text-2xl shadow-lg active:scale-95 transition-all ${settings.silentMode ? 'bg-slate-800 text-slate-400' : 'bg-red-600 text-white'}`}>
          <ShieldAlert className={`w-8 h-8 ${settings.silentMode ? 'opacity-20' : 'fill-white'}`} /> {t.emergency} 112
        </button>
      </div>

      {/* Manual Install Instruction Helper */}
      {showInstallHelp && (
        <div className="fixed inset-0 z-[60] bg-black/80 backdrop-blur-sm flex items-end justify-center p-4">
          <div className="bg-white w-full max-w-sm rounded-[2.5rem] p-8 animate-in slide-in-from-bottom duration-300">
            <div className="flex justify-between items-start mb-6">
              <h3 className="text-2xl font-black uppercase tracking-tight text-slate-900">How to Install</h3>
              <button onClick={() => setShowInstallHelp(false)} className="p-2 bg-slate-100 rounded-full"><X className="w-5 h-5"/></button>
            </div>
            <div className="space-y-6 mb-8">
              <div className="flex gap-4">
                <div className="w-8 h-8 bg-blue-100 rounded-full flex items-center justify-center text-blue-600 font-black text-sm flex-shrink-0">1</div>
                <p className="text-sm font-medium text-slate-600 leading-tight">Tap the browser menu button (3 dots on Chrome or Share icon on Safari).</p>
              </div>
              <div className="flex gap-4">
                <div className="w-8 h-8 bg-blue-100 rounded-full flex items-center justify-center text-blue-600 font-black text-sm flex-shrink-0">2</div>
                <p className="text-sm font-medium text-slate-600 leading-tight">Select <span className="font-black text-slate-900">"Install App"</span> or <span className="font-black text-slate-900">"Add to Home Screen"</span>.</p>
              </div>
              <div className="flex gap-4">
                <div className="w-8 h-8 bg-blue-100 rounded-full flex items-center justify-center text-blue-600 font-black text-sm flex-shrink-0">3</div>
                <p className="text-sm font-medium text-slate-600 leading-tight">SafeGuard will appear as a real app on your home screen.</p>
              </div>
            </div>
            <button onClick={() => setShowInstallHelp(false)} className="w-full py-4 bg-slate-900 text-white rounded-2xl font-black uppercase tracking-widest">Got it</button>
          </div>
        </div>
      )}
    </div>
        ...
        <div className="p-4 space-y-6 pb-20">
          ...
          <div className="space-y-4">
            <button 
              ...
              className="w-full bg-blue-600 text-white p-5 rounded-[2.5rem] flex items-center justify-between shadow-xl active:scale-95 transition-transform"
            >
              <div className="flex items-center gap-4">
                <div className="p-2 bg-white/20 rounded-full"><Share2 className="w-5 h-5" /></div>
                <div className="text-left">
                  <span className="font-black block text-lg">Share App</span>
            ...
            >
              <div className="flex items-center gap-4">
                <div className="p-2 bg-white/20 rounded-full"><InstallIcon className="w-5 h-5" /></div>
                <div className="text-left">
                  <span className="font-black block text-lg">Direct Install</span>
                  <span className="text-[10px] font-bold opacity-70 uppercase tracking-widest">Pin to home screen</span>
                </div>
              </div>
              <ChevronRight className="w-6 h-6 opacity-40" />
            </button>
